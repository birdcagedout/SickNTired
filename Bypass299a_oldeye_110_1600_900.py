...
class Bypass:
	def __init__(self):
		...
		self.reasons = ["", "신규등록 등 업무 대상자 확인", "이전등록 등 업무 대상자 확인", "말소압류 등 업무 대상자 확인", "소유 확인", ""]
		...
		self.radio_btn4 = Radiobutton(self.root, text="소유 확인", font=("Malgun Gothic", 12), value=4, variable=self.radio_var, command=self.on_select)
		...
		self.label_0.grid(row=0, column=0, sticky=EW, padx=7, pady=10, columnspan=1)
		...
		self.start_btn.grid(row=6, column=0, sticky=EW, padx=38, pady=18, columnspan=1)
	...
	def win_move_aside(self):
		...
		distance_x = scr_width - (pos_x + WIN_WIDTH) + 170		# X거리(여유분=50)
...
class WatcherThread(Thread):
	...
	def run(self):
		...
		while True:
			...
			try:
				# 연계정보 조회
				pos1 = pyautogui.locateCenterOnScreen(os.path.join(HOME_PATH,"inquiry_popup_oldeye_110.PNG"), grayscale=True, region=REGION)
				#pos2 = locateCenterOnScreen("inquiry_save.PNG", grayscale=True)

				# 갑부 열람사유입력
				if pos1 == None:
					gab1 = pyautogui.locateCenterOnScreen(os.path.join(HOME_PATH, "gabbu_oldeye_110.PNG"), grayscale=True, region=REGION)
			...
			if pos1 == None and gab1 == None:
				...
				if pos1 != None:
					# 조회 사유 자동입력
					pos2 = pyautogui.Point(x=pos1.x+405, y=pos1.y+40)
					#pos2 = Point(x=pos1.x+510, y=pos1.y+50)	# 강선임 주임님
					pos1 = pyautogui.Point(x=pos1.x, y=pos1.y+80)
				
				# 갑부 열람사유입력인 경우
				if gab1 != None:
					pos2 = pyautogui.Point(x=gab1.x+130, y=gab1.y+40)
