...
class Bypass:
	def __init__(self):
		...
		# "법인번호" 텍스트 라벨
		self.label_0 = Label(self.root, text="자동 입력할 조회 사유를 선택하세요", font=("Malgun Gothic", 11, 'bold'))
		...
		self.label_0.grid(row=0, column=0, sticky=EW, padx=5, pady=10, columnspan=1)
		self.radio_btn1.grid(row=1, column=0, sticky=W, padx=25, pady=0)
		self.radio_btn2.grid(row=2, column=0, sticky=W, padx=25, pady=0)
		self.radio_btn3.grid(row=3, column=0, sticky=W, padx=25, pady=0)
		self.radio_btn4.grid(row=4, column=0, sticky=W, padx=25, pady=0)
		self.radio_btn5.grid(row=5, column=0, sticky=W, padx=27, pady=0)
		self.entry_5.grid(row=5, column=0, sticky=W, padx=32, pady=0)
...
class WatcherThread(Thread):
	...
	def run(self):
		# 2.98에서 추가 ==> 45초(느린컴은 1분)마다 전원관리 timer 초기화
		timer = 0
		...
		while True:
			...
			# 둘 다 뜨는지 먼저 체크
			try:
				pos1 = pyautogui.locateCenterOnScreen(os.path.join(HOME_PATH,"inquiry_popup_motorcycle.PNG"), grayscale=True, region=REGION)
			...
			#===============================================================
			# 2.98에서 추가 ==> 45초에 1번 sleep 방지
			if timer < sys.maxsize:
				timer += 1
			else:
				timer = 0
			if timer % 300000000 == 0:
				ctypes.windll.kernel32.SetThreadExecutionState(0x80000003)
			# 2.98에서 추가
			#===============================================================
			...
			pos2 = pyautogui.Point(x=pos1.x+513, y=pos1.y+48)	# 강선임 주임님
			pos1 = pyautogui.Point(x=pos1.x, y=pos1.y+150)
