...
MY_OWN_REASON = False 	# 직접 입력한 사유가 있는 경우 True

ACTIVATED = False			# 팝업창 발견(작동 활성화)
CRASHED = False 			# 작동 에러 발생
ACTIVATION_DELAY = 0.3		# 팝업창 발견 후 작동까지 지연시간

SCREEN_WIDTH = 1920
SCREEN_HEIGHT = 1080

HOME_PATH = os.path.dirname(__file__)
VER = "3.1e"								# Pyautogui 제거, keyboard, mouse 제거, pynput으로 교체
...
class Bypass:
	def __init__(self):
		...
		# 직접 입력
		self.entry_5 = Entry(self.root, font=("Malgun Gothic", 12), fg="#707070", width=26, borderwidth=1, foreground="#090909", bg="#d7f77c")
		self.entry_5.bind("<Button-1>", self.on_input)

		# 설정 파일이 있으면 entry에 입력
		if os.path.exists("C:\sickntired.cfg") == True:
			with open("C:\sickntired.cfg", "r") as f:
				self.entry_5.insert(0, f.read())
				global MY_OWN_REASON
				MY_OWN_REASON = True
		else:
			self.entry_5.insert(0, "직접 입력(입력 후 스페이스바)")
		...
		self.scale = Scale(self.root, from_=0, to=90, resolution=10, showvalue=0, bd=1, orient=HORIZONTAL, length=187,
		...
		# 개발자 로고
		self.label_2 = Label(self.root, text="developed by 김재형, 2022-2023", font=("Malgun Gothic", 8))
		...
		self.label_0.grid(row=0, column=0, sticky=EW, padx=7, pady=(5, 5))
		...
		self.radio_btn4.grid(row=3, column=0, sticky=W, padx=35, pady=0)
		self.radio_btn5.grid(row=4, column=0, sticky=W, padx=35, pady=0)
		self.entry_5.grid(row=4, column=0, sticky=W, padx=(62,0), pady=0)
		...
		self.start_btn.grid(row=6, column=0, sticky=EW, padx=(32, 32), pady=5)
	...
	def on_start(self, event):
		...
		if choice == 0:
			...
			if len(self.reason) < 4:
				messagebox.showerror("확인", "4글자 이상 사유를 적어주세요.")
				return
			
			# 사유를 파일로 저장
			with open("C:\sickntired.cfg", "w") as f:
				f.write(self.reason)
		else:
	...
	def wait(self):
		...
		while self.root.winfo_width() > WAITING_WIN_WIDTH:
			self.root.geometry(f"{self.root.winfo_width() - 5}x{self.root.winfo_height()}")
			self.root.update()
		
		while self.root.winfo_height() > WAITING_WIN_HEIGHT:
			self.root.geometry(f"{self.root.winfo_width()}x{self.root.winfo_height() - 5}")
			self.root.update()
		...
		while True:
			...
			try:
				self.canvas.itemconfigure(image_on_canvas, image=self.smile_frames[i])
			except TclError:
				return
			self.root.update()
...
class WatcherThread(Thread):
	...
	def run(self):
		...
		while True:
			...
			#time.sleep(0.1)									# 팝업창 글자수 세는 로직의 오동작 방지를 위해 지연
			#keyboard.send('enter')
			#mouse.move(xPos, yPos)
			(xPos, yPos) = self.m.position
			self.m.position = (x + 110, y + 110)
			...
			if (title == "조회 사유 입력") or (title == "개인정보 공개 열람사유입력"):
				messagebox.showerror("에러", "팝업창이 닫히지 않았습니다.\n로그에 기록하고 종료합니다.")

				# 로그 저장
				logger = logging.getLogger()
				logger.setLevel(logging.DEBUG)
				formatter = logging.Formatter('%(asctime)s - %(message)s')
				file_handler = logging.FileHandler("C:\sickntired.log")
				file_handler.setFormatter(formatter)
				logger.addHandler(file_handler)
				logger.debug("(에러) 창이 닫히지 않았습니다.")
