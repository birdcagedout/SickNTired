...
class Bypass:
	def __init__(self):
		...
		self.radio_btn1.grid(row=1, column=0, sticky=W, padx=55, pady=0)
		self.radio_btn2.grid(row=2, column=0, sticky=W, padx=55, pady=0)
		self.radio_btn3.grid(row=3, column=0, sticky=W, padx=55, pady=0)
		self.radio_btn4.grid(row=4, column=0, sticky=W, padx=55, pady=0)
		self.radio_btn5.grid(row=5, column=0, sticky=W, padx=55, pady=0)
		self.entry_5.grid(row=5, column=0, sticky=W, padx=82, pady=0)
		...
		self.start_btn.configure(width=130, height=99, activebackground="#FFFFFF")
		#self.start_btn.place(x=24, y=230)
		self.start_btn.grid(row=6, column=0, sticky=EW, padx=69, pady=18, columnspan=1)
		...
		if self.target_win_hwnd == None:
			messagebox.showerror("오류 발생", "이륜차관리정보시스템이 시작되지 않았습니다.\n프로그램을 종료합니다.")
			sys.exit(0)
	...
	def win_move_aside(self):
		...
		distance_x = scr_width - (pos_x + WIN_WIDTH) + 230		# X거리(여유분=50)
		distance_y = scr_height - (pos_y + WIN_HEIGHT) + 150  	# Y거리(여유분=100)
...
class WatcherThread(Thread):
	...
	def run(self):
		...
		while True:
			...
			try:
				# 연계정보 조회
				pos1 = pyautogui.locateCenterOnScreen(os.path.join(HOME_PATH,"inquiry_popup_oldeye.PNG"), grayscale=True, region=REGION)
				#pos2 = locateCenterOnScreen("inquiry_save.PNG", grayscale=True)

				# 갑부 열람사유입력
				if pos1 == None:
					gab1 = pyautogui.locateCenterOnScreen(os.path.join(HOME_PATH, "gabbu_oldeye.PNG"), grayscale=True, region=REGION)
					

			except OSError as ose:
				#messagebox.showerror("OSError 발생", "commandline 확인 필요")
				#sys.exit(0)
				#print(f"[locateCenterOnScreen 에러] 종류:{type(ose)}\t 내용:{ose}")
				#print(f"Traceback:{traceback.print_exc()}")
				#global pyautogui
				#importlib.reload(pyautogui)
				THREAD_DEAD = True
				#time.sleep(2)
				#print("sleep 2초 후 쓰레드 죽습니다")
				break

			except Exception as e:
				#print(f"[locateCenterOnScreen 에러] 종류:{type(e)}\t 내용:{e}")
				#print(f"Traceback:{traceback.print_exc()}")
				#time.sleep(2)
				#importlib.reload(pyautogui)
				THREAD_DEAD = True
			...
			if pos1 == None and gab1 == None:
				...
				if pos1 != None:
					# 조회 사유 자동입력
					pos2 = pyautogui.Point(x=pos1.x+505, y=pos1.y+40)
					#pos2 = Point(x=pos1.x+510, y=pos1.y+50)	# 강선임 주임님
					pos1 = pyautogui.Point(x=pos1.x, y=pos1.y+100)
				
				# 갑부 열람사유입력인 경우
				if gab1 != None:
					pos2 = pyautogui.Point(x=gab1.x+165, y=gab1.y+45)
					pos1 = pyautogui.Point(x=gab1.x, y=gab1.y+120)
