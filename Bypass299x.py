...
VER = "2.99x10"								# Pyautogui 제거, keyboard 모듈 추가, mouse 모듈 추가
...
class Bypass:
	def __init__(self):
		...
		self.root.tk.call('tk', 'scaling', self.root.winfo_fpixels('1i')/72)			# 해상도와 무관하게 DPI scaling = 96/72
		...
		self.label_0.grid(row=0, column=0, sticky=EW, padx=5, pady=(10, 5))
		...
		self.scale.grid(row=5, column=0, sticky=W, padx=(110, 0), pady=(3,5))
		...
		self.start_btn = Button(self.root, text="고고씽ㅋ~", font=("Malgun Gothic", 36, 'bold'), foreground="#FFFFFF", background="#208DDE", activeforeground="#DDDDDD", activebackground="#55b6fd", borderwidth=1)
		self.start_btn.bind("<ButtonRelease-1>", self.on_start)
		#self.start_btn.configure(justify='center')
		self.start_btn.grid(row=6, column=0, sticky=EW, padx=(30, 30), pady=10)
		...
		if self.target_win_hWnd == None:
			messagebox.showerror("오류 발생", "자동차(이륜차)관리정보시스템이 시작되지 않았습니다.\n프로그램을 종료합니다.")
			sys.exit(0)
